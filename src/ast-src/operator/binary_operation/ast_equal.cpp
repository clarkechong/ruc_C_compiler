...
Equal::Equal(NodePtr left, NodePtr right)
    : BinaryOperator(std::move(left), std::move(right))
{
}

void Equal::EmitRISCV(std::ostream& stream, const std::string& dst_reg, Context& context) const 
{
    ...
    stream << "    sub " << dst_reg << ", " << left_reg << ", " << right_reg << std::endl;
    stream << "    seqz " << dst_reg << ", " << dst_reg << std::endl;
    // ^^ INSERT SNEZ IN FUTURE IF DEALING WITH FLOATS!!!
    
    context.register_manager.DeallocateRegister(left_reg);
    context.register_manager.DeallocateRegister(right_reg);
}

void Equal::Print(std::ostream& stream, indent_t indent) const 
{
    stream << indent << "Equal [" << std::endl;
